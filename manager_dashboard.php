...
$summary = array('revenue' => 0, 'cost' => 0, 'profit' => 0, 'qty' => 0, 'orders' => 0);
$daily = array();
$topRows = array();
$bottomRows = array();
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($out, array('Выручка', 'Себестоимость', 'Валовая прибыль', 'Маржа %', 'Продано метров', 'Операций'), ';');
    fputcsv($out, array(
    ...
    fputcsv($out, array(), ';');
    fputcsv($out, array('Товар', 'Продано, м', 'Выручка', 'Себестоимость', 'Прибыль', 'Маржа %'), ';');
...
<main class="container">
    ...
    <div class="stats-grid">
        <div class="stat-card"><div class="stat-number"><?= number_format($sumRevenue, 0, '.', ' ') ?></div><div>Выручка, KGS</div></div>
...
(function () {
    ...
    var labels = [], revenue = [], cost = [], profit = [];
    ...
    if (canvas && typeof Chart !== 'undefined') {
        new Chart(canvas, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: 'Выручка', data: revenue, borderColor: '#2E86DE', tension: 0.2 },
